...
$$
\hat{\theta}_{\mathrm{MLE}} = \arg\max_{\theta} \log p(\mathcal{D}\mid \theta) = \arg\min_{\theta} \big(-\log p(\mathcal{D}\mid \theta)\big).
$$

因此，**MLE 的本质就是最小化负对数似然（negative log-likelihood）。换句话说，MLE的本质是找到一组参数，使得在这组参数下，观测到的数据出现的概率最大**
...
$$
\mathbf{y} = X\boldsymbol{\theta} + \boldsymbol{\epsilon}, \qquad \boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0},\sigma^2 I),
$$

其中：

- $\mathbf{y}\in\mathbb{R}^n$ 为观测向量；
- $X\in\mathbb{R}^{n\times d}$ 为设计矩阵；
- $\boldsymbol{\theta}\in\mathbb{R}^d$ 为待估参数；
- $\boldsymbol{\epsilon}$ 为高斯噪声。
...
$$
\mathbf{y}\mid \boldsymbol{\theta} \sim \mathcal{N}(X\boldsymbol{\theta},\,\sigma^2 I).
$$

因此其概率密度函数为

$$
p(\mathbf{y}\mid \boldsymbol{\theta}) = \frac{1}{(2\pi \sigma^2)^{n/2}} \exp\left(-\frac{1}{2\sigma^2} (\mathbf{y}-X\boldsymbol{\theta})^T(\mathbf{y}-X\boldsymbol{\theta})\right).
$$

取对数得

$$
\log p(\mathbf{y}\mid \boldsymbol{\theta}) = -\frac{n}{2}\log(2\pi\sigma^2) - \frac{1}{2\sigma^2} (\mathbf{y}-X\boldsymbol{\theta})^T(\mathbf{y}-X\boldsymbol{\theta}).
$$

由于第一项与 $\boldsymbol{\theta}$ 无关，因此最大化对数似然等价于最小化

$$
(\mathbf{y}-X\boldsymbol{\theta})^T(\mathbf{y}-X\boldsymbol{\theta}) = \|\mathbf{y}-X\boldsymbol{\theta}\|_2^2.
$$

于是得到

$$
\hat{\boldsymbol{\theta}}_{\mathrm{MLE}} = \arg\min_{\boldsymbol{\theta}} \|\mathbf{y}-X\boldsymbol{\theta}\|_2^2.
$$
...
$$
-\log p(\mathcal{D}\mid \theta) = \text{常数项} + \frac{1}{2\sigma^2} \sum_{i=1}^n (y_i-f_{\theta}(x_i))^2.
$$

因此

$$
\hat{\theta}_{\mathrm{MLE}} = \arg\min_{\theta} \sum_{i=1}^n (y_i-f_{\theta}(x_i))^2.
$$
...
$$
\hat{\theta}_{\mathrm{MAP}} = \arg\max_{\theta} \big[ \log p(\mathcal{D}\mid \theta) + \log p(\theta) \big].
$$
...
$$
\boxed{L_{\mathrm{MAP}}(\theta) = -\log p(\mathcal{D}\mid \theta) - \log p(\theta)}
$$
...
$$
\boldsymbol{\theta} \sim \mathcal{N}(\mathbf{0},\tau^2 I).
$$

则先验密度为

$$
p(\boldsymbol{\theta}) = \frac{1}{(2\pi\tau^2)^{d/2}} \exp\left(-\frac{1}{2\tau^2} \boldsymbol{\theta}^T\boldsymbol{\theta}\right).
$$

取负对数：

$$
-\log p(\boldsymbol{\theta}) = \frac{d}{2}\log(2\pi\tau^2) + \frac{1}{2\tau^2} \|\boldsymbol{\theta}\|_2^2.
$$

忽略常数项：

$$
-\log p(\boldsymbol{\theta}) \equiv \frac{1}{2\tau^2} \|\boldsymbol{\theta}\|_2^2.
$$

因此 MAP 目标函数为

$$
L_{\mathrm{MAP}}(\boldsymbol{\theta}) = \frac{1}{2\sigma^2}\|\mathbf{y}-X\boldsymbol{\theta}\|_2^2 + \frac{1}{2\tau^2}\|\boldsymbol{\theta}\|_2^2.
$$

两边同乘 $2\sigma^2$，不改变最优解，得

$$
\hat{\boldsymbol{\theta}}_{\mathrm{MAP}} = \arg\min_{\boldsymbol{\theta}} \left[ \|\mathbf{y}-X\boldsymbol{\theta}\|_2^2 + \lambda \|\boldsymbol{\theta}\|_2^2 \right],
$$
...
$$
\boxed{\text{Gaussian likelihood} + \text{Gaussian prior} \Longrightarrow \text{MSE} + L_2 \text{ regularization}}
$$
...
$$
p(\theta_j) = \frac{\lambda}{2}\exp(-\lambda |\theta_j|), \qquad j=1,\dots,d.
$$

则联合先验为

$$
p(\boldsymbol{\theta}) = \prod_{j=1}^d \frac{\lambda}{2}\exp(-\lambda |\theta_j|) = \left(\frac{\lambda}{2}\right)^d \exp\left(-\lambda \sum_{j=1}^d |\theta_j|\right).
$$

注意

$$
\sum_{j=1}^d |\theta_j| = \|\boldsymbol{\theta}\|_1.
$$

因此

$$
p(\boldsymbol{\theta}) = \left(\frac{\lambda}{2}\right)^d \exp(-\lambda \|\boldsymbol{\theta}\|_1).
$$

取负对数得

$$
-\log p(\boldsymbol{\theta}) = -d\log\frac{\lambda}{2} + \lambda \|\boldsymbol{\theta}\|_1.
$$

忽略常数项：

$$
-\log p(\boldsymbol{\theta}) \equiv \lambda \|\boldsymbol{\theta}\|_1.
$$

于是 MAP 目标函数变为

$$
L_{\mathrm{MAP}}(\boldsymbol{\theta}) = \frac{1}{2\sigma^2}\|\mathbf{y}-X\boldsymbol{\theta}\|_2^2 + \lambda \|\boldsymbol{\theta}\|_1.
$$

乘以 $2\sigma^2$ 后可重写为

$$
\hat{\boldsymbol{\theta}}_{\mathrm{MAP}} = \arg\min_{\boldsymbol{\theta}} \left[ \|\mathbf{y}-X\boldsymbol{\theta}\|_2^2 + \lambda' \|\boldsymbol{\theta}\|_1 \right],
$$
...
$$
\boxed{\text{Gaussian likelihood} + \text{Laplace prior} \Longrightarrow \text{MSE} + L_1 \text{ regularization}}
$$
...
$$
\boxed{\text{参数的整个后验分布 } p(\theta\mid \mathcal{D})}
$$
...
$$
p(y^\ast|x^\ast,\mathcal D) = \int p(y^\ast|x^\ast,\theta)p(\theta|\mathcal D)\,d\theta,
$$
...
$$
a^\ast = \arg\min_{a} \mathbb{E}_{\theta\mid \mathcal{D}} \big[ L(a,\theta) \big] = \arg\min_a \int L(a,\theta)\,p(\theta\mid \mathcal{D})\,d\theta.
$$
...
$$
R(a) = \int (a-\theta)^2 p(\theta\mid \mathcal{D})\,d\theta.
$$

对 $a$ 求导：

$$
\frac{dR(a)}{da} = \int 2(a-\theta)\,p(\theta\mid \mathcal{D})\,d\theta = 2a\int p(\theta\mid \mathcal{D})\,d\theta - 2\int \theta p(\theta\mid \mathcal{D})\,d\theta.
$$
...
$$
\frac{dR(a)}{da} = 2a - 2\mathbb{E}[\theta\mid \mathcal{D}].
$$
...
$$
\boxed{\text{平方损失} \Longrightarrow \text{后验均值}}
$$
...
$$
R(a) = \int |a-\theta|\,p(\theta\mid \mathcal{D})\,d\theta.
$$

将积分拆为两部分：

$$
R(a) = \int_{-\infty}^{a} (a-\theta)\,p(\theta\mid \mathcal{D})\,d\theta + \int_{a}^{\infty} (\theta-a)\,p(\theta\mid \mathcal{D})\,d\theta.
$$

对 $a$ 求导：

$$
\frac{dR(a)}{da} = \int_{-\infty}^{a} p(\theta\mid \mathcal{D})\,d\theta - \int_{a}^{\infty} p(\theta\mid \mathcal{D})\,d\theta.
$$

令导数为零，得

$$
\int_{-\infty}^{a^\ast} p(\theta\mid \mathcal{D})\,d\theta = \int_{a^\ast}^{\infty} p(\theta\mid \mathcal{D})\,d\theta = \frac{1}{2}.
$$

这正说明 $a^\ast$ 是后验分布的中位数。因此

$$
\boxed{\text{绝对损失} \Longrightarrow \text{后验中位数}}
$$
...
$$
R(a) = \int L(a,\theta)p(\theta\mid \mathcal{D})\,d\theta = 1 - p(a\mid \mathcal{D})
$$
...
$$
a^\ast = \arg\max_a p(a\mid \mathcal{D}).
$$

这就是后验众数，即 MAP：

$$
\boxed{\text{$0$-$1$ 损失} \Longrightarrow \text{后验众数（MAP）}}
$$
...
$$
\boxed{\text{MLE: 数据拟合} \quad\subset\quad \text{MAP: 数据拟合 + 先验约束} \quad\subset\quad \text{Bayes: 完整后验分布与决策}}
$$
